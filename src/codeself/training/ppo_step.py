"""Optional Torch optimizer step for PPO tensor batches."""
...
class PPOOptimizerStepConfig:
    """Settings for one PPO optimizer step."""
...
@dataclass(frozen=True)
class PPOOptimizerStepResult:
    """Detached diagnostics from one PPO optimizer step."""
...
def run_ppo_optimizer_step(
    batch: PPOTensorBatch,
    *,
    optimizer: Any,
    config: PPOOptimizerStepConfig | None = None,
) -> PPOOptimizerStepResult:
    """Backpropagate PPO tensor loss and optionally step an optimizer."""

    torch = require_torch()
    step_config = config or PPOOptimizerStepConfig()
    _validate_optimizer(optimizer)
    if step_config.zero_grad:
        _zero_grad(optimizer)

    loss_result = compute_ppo_tensor_loss(batch, config=step_config.loss)
    ...
    return PPOOptimizerStepResult(
        ...
        value_loss=_detach_float(loss_result.value_loss),
        entropy_loss=_detach_float(loss_result.entropy_loss),
        ...
        mean_advantage=loss_result.mean_advantage,
        mean_return=loss_result.mean_return,
        mean_entropy=loss_result.mean_entropy,
        mean_approx_kl=loss_result.mean_approx_kl,
        clipped_policy_fraction=loss_result.clipped_policy_fraction,
        clipped_value_fraction=loss_result.clipped_value_fraction,
    )
