...
class PPORolloutBatchTests(unittest.TestCase):
    def test_builds_batch_from_raw_completion_without_assigning_advantages(self) -> None:
        ...
        result = build_ppo_training_batch_from_rollouts(
            records,
            tokenizer,
            config=PPORolloutBatchConfig(max_prompt_tokens=3, max_response_tokens=8),
        )
        ...
        self.assertIsNone(result.batch.samples[0].advantage)
    ...
    def test_can_train_on_parsed_code_and_skip_failed_parses(self) -> None:
        ...
        self.assertIsNone(sample.advantage)
    ...
    def test_config_validates_ranges_and_response_source(self) -> None:
        with self.assertRaises(ValueError):
            PPORolloutBatchConfig(max_prompt_tokens=0)
        with self.assertRaises(ValueError):
            PPORolloutBatchConfig(max_response_tokens=-1)
        with self.assertRaises(ValueError):
            PPORolloutBatchConfig(response_source="completion")  # type: ignore[arg-type]
